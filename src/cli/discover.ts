...
async function main() {
  ...
  if (!urlArg) {
    console.error(
      'Usage: npm run discover -- --url=https://example.com [--name=example] [--depth=2] [--max-pages=50] [--output=projects/example/discovered] [--headless=true]',
    );
    process.exit(1);
  }
  ...
  const result = await engine.discover({ url, depth, maxPages, outputDir, name, headless });
